...
import SortableSolution from './SortableSolution';

const SolutionsBuilder = () => {
  ...
  useEffect(() => {
    const jsonItems = data ? JSON.parse(data?.solutions?.param1) : [];
    setItems(jsonItems);
  }, [data]);
  ...
  const handleSubmit = () => {
    ...
    repeaterUpdate('solutions', params, setActionLoader);
  };
  ...
  return (
    <>
      <SortableList
        className="list"
        itemComponent={SortableSolution}
...
export default SolutionsBuilder;
